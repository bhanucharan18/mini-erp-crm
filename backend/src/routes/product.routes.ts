...
import authMiddleware, { AuthRequest, authorize } from "../middleware/auth.middleware.js";
 ...
 * CREATE PRODUCT
 */
router.post("/", authMiddleware, authorize(["ADMIN", "WAREHOUSE"]), async (req: AuthRequest, res) => {
  try {
    const {
      name,
      sku,
 ...
 * UPDATE PRODUCT
 */
router.put("/:id", authMiddleware, authorize(["ADMIN", "WAREHOUSE"]), async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const {
      name,
 ...
 * DELETE PRODUCT
 */
router.delete("/:id", authMiddleware, authorize(["ADMIN", "WAREHOUSE"]), async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
