  ...
  try {
    ...
    if (!name || !mobile || !customerType || !address) {
      return res.status(400).json({
        success: false,
        message:
          "Name, Mobile, Customer Type and Address are required",
      });
    }
 ...
 * UPDATE CUSTOMER
 */
router.put("/:id", authMiddleware, authorize(["ADMIN", "SALES"]), async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const {
      name,
 ...
 * ADD CRM FOLLOW-UP
 */
router.post("/:id/follow-ups", authMiddleware, authorize(["ADMIN", "SALES"]), async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
